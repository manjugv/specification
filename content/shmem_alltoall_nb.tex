...
\apidescription{
    ...
    an opaque request handle is created and returned. The completion of the
    operation can be observed after one or more calls to \FUNC{shmem\_req\_test} or a
    ...
    \begin{itemize}
    \item The \VAR{source} data object on the local \ac{PE} in the team is
      ready to be read by any \ac{PE} in the team.
    \end{itemize}
...
\apinotes{
    It is the implementation's responsibility to ensure the \dest{} array on
    all \acp{PE} in the team are ready to accept the \FUNC{shmem\_alltoall\_nb}
    data before updates may occur.
}

\begin{apiexamples}

\apicexample
    {This \CorCpp{} example shows concurrent nonblocking \FUNC{shmem\_int64\_alltoall\_nb} on two 64-bit integers among all \acp{PE}.}
